...
/// <summary>
/// Tests that a typed config derived from <see cref="IJobConfig"/> is correctly
/// propagated to the job via <see cref="IJobRunnerConfigAccessor{TConfig}"/>.
/// </summary>
public class IntervalJobConfigAccessorTest
{
    [Fact]
    public async Task TypedConfigShouldBeInjectedIntoJob()
    {
        const string expectedTag = "my-interval-tag";
        var config = new MockIntervalJobConfig
        {
    ...
    private static (IHostedService Scheduler, ConfigCaptureStore Store, FakeTimeProvider TimeProvider, ServiceProvider Services) BuildScheduler(MockIntervalJobConfig config)
    {
        var sc = new ServiceCollection();
        sc.AddLogging();
        sc.AddSingleton<ConfigCaptureStore>();
        sc.AddScheduledJob<ConfigAwareIntervalJob, MockIntervalJobConfig>(config);
        sc.AddMockedClock();
