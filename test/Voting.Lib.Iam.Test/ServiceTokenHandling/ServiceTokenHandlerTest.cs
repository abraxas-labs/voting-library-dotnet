...
public class ServiceTokenHandlerTest
{
    ...
    public async Task GetTokenShouldWork()
    {
        ...
        var token = await handler.GetServiceToken();
    ...
    private void ExpectTokenFetch(MockHttpMessageHandler handler, string token)
    {
        handler
            ...
            .Respond(JsonContent.Create(new AccessTokenResponse { ExpiresIn = 600, AccessToken = token }, options: SecureConnectDefaults.JsonOptions));
    }
}
