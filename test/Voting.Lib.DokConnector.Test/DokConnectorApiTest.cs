...
public class DokConnectorApiTest
{
    ...
    {
        using var client = MockClient(new ConnectContentMatcher("fooBar", "file.txt", "my-message-type"));
        var connector = new DokConnectorApi(client, NullLogger<DokConnectorApi>.Instance);
    ...
    private class ConnectContentMatcher : IMockedRequestMatcher
    {
        ...
        public ConnectContentMatcher(string expectedFileContent, string expectedFileName, string expectedMessageType)
        {
